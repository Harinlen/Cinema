...
private:
    explicit KNDpiManager(QObject *parent = nullptr);
    KNDpiManager(const KNDpiManager &);
    KNDpiManager(KNDpiManager &&);
    inline int scaleX(int rawWidth)
    {
        return static_cast<int>(scaleXF(static_cast<qreal>(rawWidth)));
    }
    ...
    {
        return static_cast<int>(scaleYF(static_cast<qreal>(rawHeight)));
    }
